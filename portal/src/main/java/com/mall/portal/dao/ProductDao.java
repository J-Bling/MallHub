...
public interface ProductDao {
     ...
     */
    @Select("select id,create_at from pms_product where new_status = 1 and verify_status =1 order by create_at desc limit #{offset},#{limit}")
    List<PmsProduct> getMaxCreateProduct(@Param("offset") int offset,@Param("limit") int limit);
    /**
     * 获取按销量排行前 n个 个商品
     */
    @Select("select id,sale from pms_product order by sale desc limit #{offset},#{limit}")
